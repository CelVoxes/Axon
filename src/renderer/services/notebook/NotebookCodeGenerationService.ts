...
export class NotebookCodeGenerationService {
	...
	private seededNotebooks: Set<string> = new Set<string>();
	private static installCellCache: Map<string, boolean> = new Map<
		string,
		boolean
	>();
	...
	private async seedContextFromNotebook(notebookPath: string): Promise<void> {
		try {
			if (this.seededNotebooks.has(notebookPath)) return;
			...
			this.seededNotebooks.add(notebookPath);
		} catch (e) {
	...
	private async ensureRSetupCell(
		...
		try {
			...
			const notebookContent = await this.notebookService.readNotebook(
				notebookPath
			);
			...
			const hasRSetup =
				/(install\.packages\(|library\(Seurat\)|Seurat::)/i.test(existingCode);
			if (hasRSetup) return;
	...
	async generateAndAddValidatedCode(args: GenerateAndAddArgs): Promise<void> {
		...
		const stepId = `nbcg-${Date.now()}-${Math.random()
			.toString(36)
			.substr(2, 6)}`;
		...
		if (isSpectralFlow) {
			...
			if (
				typeof (this.codeGenerator as any).emitValidationSuccess === "function"
			) {
				(this.codeGenerator as any).emitValidationSuccess(
					stepId,
					"Generated R (Seurat) code added without Python linting",
					generatedCode
				);
			}
			return;
		...
		let validation;
		try {
			validation = await this.codeQualityService.validateAndTest(
				generatedCode,
				stepId,
				{
		...
		// Keep cells self-contained: do not strip imports based on global context
	...
	private async ensurePackageInstallationCell(
		...
		try {
			console.log("🔍 ensurePackageInstallationCell called with:", {
				notebookPath,
			...
			// If package installation already exists, skip and cache
			if (hasInstall) {
				console.log(
					"⏭️ Skipping package installation - already exists in notebook"
				);
				NotebookCodeGenerationService.installCellCache.set(notebookPath, true);
				return;
			}

			// Generate package installation code
			console.log("🔧 Generating package installation code...");
			const packageInstallCode =
			...
			let finalInstallCode =
				packageInstallCode && packageInstallCode.trim().length > 0
					? packageInstallCode
					: undefined;
			...
			if (
				...
				console.log(
					"📦 Falling back to LLM-suggested packages for install cell:",
					llmSuggestedPkgs
				);
