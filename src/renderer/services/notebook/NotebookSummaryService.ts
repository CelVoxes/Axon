...
export class NotebookSummaryService {
	...
	processCells(
		...
		const selectedCells = selectedIndices
			...
			if (cell.outputs && Array.isArray(cell.outputs)) {
				cell.outputs.forEach((output) => {
					if (output.output_type === "stream" && output.text) {
						...
						if (textPlain) {
							const textContent = Array.isArray(textPlain)
								? textPlain.join("")
								: textPlain;
						...
						if (output.data["text/html"]) {
							...
							if (
								...
								htmlContent.includes("dataframe")
							) {
								visualOutputs.push({
									type: "table",
									format: "html",
	...
	private extractPlotInsights(code: string): string {
		...
		if (code.includes("plt.") || code.includes("matplotlib"))
			insights.push("matplotlib");
		if (code.includes("sns.") || code.includes("seaborn"))
			insights.push("seaborn");
	 ...
	 */
	private extractSavedFigures(
		processedCells: ProcessedCell[],
		workspaceDir: string
	): Array<{ filename: string; fullPath: string; description: string }> {
		const figureFiles: Array<{
			filename: string;
			fullPath: string;
			description: string;
		}> = [];
		...
		processedCells.forEach((cell) => {
			...
			const saveFigMatches = cell.content.match(
				/(?:plt\.savefig|savefig)\s*\(\s*['"]([^'"]+\.(?:png|jpg|jpeg|pdf|svg))['"].*?\)/gi
			);
			if (saveFigMatches) {
			...
			if (cell.outputs) {
				cell.outputs.forEach((output) => {
					...
					const filenameMatches = output.match(
						/([a-zA-Z0-9_-]+\.(?:png|jpg|jpeg|pdf|svg))/g
					);
					if (filenameMatches) {
					...
					const savedMatches = output.match(
						/(?:saved|written|created).*?(?:as|to)\s+([a-zA-Z0-9_./\\-]+\.(?:png|jpg|jpeg|pdf|svg))/gi
					);
					if (savedMatches) {
						savedMatches.forEach((match) => {
							const filenameMatch = match.match(
								/([a-zA-Z0-9_./\\-]+\.(?:png|jpg|jpeg|pdf|svg))/i
							);
							if (
								filenameMatch &&
								!figureFiles.some((f) => f.filename === filenameMatch[1])
							) {
	...
	private async loadImageAsBase64(filePath: string): Promise<string | null> {
		try {
			...
			const fileExists = await (window as any).electronAPI?.getFileInfo?.(
				filePath
			);
	...
	private generateReportPath(
		...
		const workspaceDir = notebookPath.substring(
			0,
		...
		// Get file extension
		const extension =
			format === "html" ? ".html" : format === "pdf" ? ".pdf" : ".md";
	 ...
	 */
	private extractQuantitativeResults(
		processedCells: ProcessedCell[]
	): string[] {
			...
			if (cell.visualOutputs) {
				cell.visualOutputs.forEach((visual) => {
					if (visual.type === "table" && visual.data) {
						...
						const tableText =
							typeof visual.data === "string" ? visual.data : "";
						if (tableText.includes("dtype:")) {
							// DataFrame info
							const lines = tableText.split("\n");
							const dataTypeInfo = lines.find((line) =>
								line.includes("dtype:")
							);
	...
	analyzeNotebook(cells: NotebookCell[], filePath: string): NotebookAnalysis {
		const title =
			filePath.split("/").pop()?.replace(".ipynb", "") || "Notebook Analysis";
		const codeCount = cells.filter((cell) => cell.cell_type === "code").length;
		const markdownCount = cells.filter(
			(cell) => cell.cell_type === "markdown"
		).length;
		const hasOutputs = cells.some(
			(cell) => cell.outputs && cell.outputs.length > 0
		);
			...
			if (cell.cell_type === "code" && cell.outputs) {
				cell.outputs.forEach((output) => {
					if (
						...
						if (
							textStr.includes("accuracy") ||
							textStr.includes("precision") ||
							textStr.includes("recall") ||
							textStr.includes("correlation") ||
							textStr.includes("%") ||
							/\d+\.\d+/.test(textStr)
						) {
	...
	async generateSummary(
		...
		const summaryContent = await this.generateAISummary(
			context,
			options,
			sessionId,
			onStream
		...
		const figuresWithPaths = this.extractSavedFigures(
			processedCells,
			workspaceDir
		);
		...
		return {
			title: `${analysis.title} - ${this.getReportTypeLabel(
				options.reportType
			)}`,
			...
			fileName,
			savedFigures:
				savedFiguresWithData.length > 0 ? savedFiguresWithData : undefined,
		};
	}
	...
	async generateAndExportSummary(
		...
		exportResult: { success: boolean; filePath?: string };
	}> {
	...
	private buildAnalysisContext(
		...
		sections.push(
			`${processedCells.length} cells selected, ${
				analysis.hasOutputs ? "with outputs" : "no outputs"
			}`
		);
	...
	private buildPrompt(
		...
		const typePrompts = {
			"quick-summary":
				"Create a bullet-point summary focusing on data, methods, results, and key figures.",
			"research-report":
				"Create a research report with Introduction, Methods, Results, Discussion, and Conclusion.",
			"technical-doc":
				"Create technical documentation covering implementation details and methodology.",
		};
	...
	private formatSummary(
		...
		let formatted = `# ${analysis.title} - ${this.getReportTypeLabel(
			options.reportType
		)}\n\n`;
		...
		formatted += `- **Source notebook:** \`${notebookPath
			.split("/")
			.pop()}\`\n`;
		formatted += `- **Report type:** ${this.getReportTypeLabel(
			options.reportType
		)}\n`;
		...
		formatted += `- **Contains executed outputs:** ${
			analysis.hasOutputs ? "Yes" : "No"
		}\n`;
		...
		formatted += `\n### Content Options\n`;
		formatted += `- **Include code:** ${
			options.includeCode ? "✓ Yes" : "✗ No"
		}\n`;
		formatted += `- **Include outputs:** ${
			options.includeOutputs ? "✓ Yes" : "✗ No"
		}\n`;
		formatted += `- **Include figures:** ${
			options.includeFigures ? "✓ Yes" : "✗ No"
		}\n`;
		formatted += `- **Include tables:** ${
			options.includeTables ? "✓ Yes" : "✗ No"
		}\n`;
		formatted += `- **Summary length:** ${this.getSummaryLengthLabel(
			options.summaryLength
		)}\n`;
	...
	private getFallbackSummary(options: SummaryOptions): string {
		return `# Summary Generation Failed\n\nCouldn't generate AI summary. Selected ${
			options.selectedCells.length
		} cells for ${this.getReportTypeLabel(options.reportType).toLowerCase()}.`;
	}
	...
	async exportSummary(
		...
		try {
			...
			if (!finalOutputPath) {
				throw new Error(
					"No output path specified and no suggested path available"
				);
			}

			// Ensure reports directory exists
			const reportsDir = finalOutputPath.substring(
				0,
				finalOutputPath.lastIndexOf("/")
			);
			...
			if (summary.format === "html") {
				finalContent = this.convertMarkdownToHTML(
					summary.content,
...
${htmlContent}
			...
			if (result?.success) {
				...
				console.warn(
					"PDF generation failed, saving as HTML with .pdf extension"
				);
				const fallbackResult = await (window as any).electronAPI.writeFile(
					outputPath,
					pdfHtml
				);
