...
Focus on the specific biological analysis they're asking for. Be precise and actionable.`,
    ...
    async generateQuestionSpecificSteps(understanding, datasets, workingDir) {
        ...
        for (let i = 0; i < understanding.requiredSteps.length; i++) {
            ...
            try {
                const code = await this.generateStepCode(stepDescription, understanding.userQuestion, datasets, workingDir, i);
                steps.push({
                    id: `step_${i + 1}`,
    ...
    async generateStepCode(stepDescription, originalQuestion, datasets, workingDir, stepIndex) {
        try {
...
`;
    ...
    async generateDynamicCode(step, analysisResult) {
        ...
        return await this.generateStepCode(step.description, analysisResult.understanding.userQuestion, analysisResult.datasets, analysisResult.workingDirectory, parseInt(step.id.split("_")[1]) - 1);
    }
    stopAnalysis() {
