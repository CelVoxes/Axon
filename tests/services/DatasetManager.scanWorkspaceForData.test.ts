...
class MockFS implements FilesystemAdapter {
  ...
  async getFileInfo(filePath: string): Promise<{ isDirectory: boolean; size?: number }> {
...
describe('DatasetManager.scanWorkspaceForData', () => {
  it('detects 10x folders and CSV/TSV/VCF files', async () => {
    ...
    fs.setDir('/ws/data', [
      ...
      { name: 'single_cell.csv', isDirectory: false, path: '/ws/data/single_cell.csv' },
      { name: 'expr.tsv', isDirectory: false, path: '/ws/data/expr.tsv' },
      { name: 'variants.vcf', isDirectory: false, path: '/ws/data/variants.vcf' },
    ]);

    // 10x directory contents
    fs.setDir('/ws/data/pbmc10k', [
      { name: 'matrix.mtx', isDirectory: false, path: '/ws/data/pbmc10k/matrix.mtx' },
      { name: 'features.tsv', isDirectory: false, path: '/ws/data/pbmc10k/features.tsv' },
      { name: 'barcodes.tsv', isDirectory: false, path: '/ws/data/pbmc10k/barcodes.tsv' },
    ]);
    ...
    // CSV single-cell
    const csv = datasets.find(d => d.localPath === '/ws/data/single_cell.csv');
